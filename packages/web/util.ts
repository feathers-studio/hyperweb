...
export function parseAsValidSubpath(root: string, path: string) {
...
export function readEnv(key: string, default_value?: string) {
	const value = Bun.env[key];
	if (value === undefined) {
		if (default_value === undefined) throw new Error(`Environment variable ${key} is not set`);
